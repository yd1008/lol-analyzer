...
def js_i18n_payload(locale: str | None = None) -> dict:
    ...
    return {
        ...
        'labels': {
            ...
            'streamFallback': lt('Live stream interrupted. Falling back to standard analysis...', '实时流中断，正在切换为标准分析...', locale=lang),
            'streamUnavailable': lt('Live stream is unavailable in this browser. Running standard analysis...', '当前浏览器不支持实时流，正在执行标准分析...', locale=lang),
            'staleFallback': lt('Live stream returned cached analysis. Retrying with standard analysis...', '实时流返回了缓存分析，正在使用标准分析重试...', locale=lang),
