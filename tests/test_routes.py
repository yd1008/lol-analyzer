...
class TestLandingPage:
    ...
    def test_landing_has_mobile_stack_hooks_for_hero_actions(self, client):
        ...
        assert b'class="hero-cta-track"' in resp.data
        assert b'class="hero-cta-buttons cta-buttons hero-mobile-stack"' in resp.data
        assert b'id="hero-primary-cta" class="btn btn-primary hero-primary-cta" aria-describedby="landing-cta-support"' in resp.data
        assert b'id="hero-secondary-cta" class="btn btn-secondary hero-secondary-cta"' in resp.data
...
class TestAiAnalysisRoute:
    ...
    def test_ai_analysis_stream_retries_sync_on_initial_stream_error_for_non_general_focus(self, auth_client, db, user):
        match = MatchAnalysis(
            user_id=user.id,
            match_id="NA1_stream_retry_focus",
            champion="Ahri",
            win=True,
            kills=5,
            deaths=2,
            ...
            game_duration=30.0,
            recommendations=[],
            llm_analysis="existing general cache",
            llm_analysis_en="existing general cache",
            queue_type="Ranked Solo",
            participants_json=[
                {"is_player": True, "team_id": 100, "position": "MIDDLE", "champion": "Ahri"},
                {"is_player": False, "team_id": 200, "position": "MIDDLE", "champion": "Syndra"},
...
class TestMatchDetailRoute:
    ...
    def test_match_detail_handles_null_gold_total(
        ...
        assert b'id="detail-tabs" role="tablist" aria-orientation="horizontal"' in resp.data
        assert b'id="detail-visual-toggle" role="tablist" aria-orientation="horizontal"' in resp.data
        ...
        assert b'<th scope="row" class="summary-row-header">' in resp.data
